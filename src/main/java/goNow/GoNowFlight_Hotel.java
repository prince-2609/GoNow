...
public class GoNowFlight_Hotel 
{
	...
	{
		return QaDataProvider.getTestdata("goNowFlight+Hotel","Sheet1");
	}
			...
			String BillingCountry,String BillingCity) throws Exception 
	{
		...
		if (UserType.equalsIgnoreCase("LogIn"))
		{
			...
			GoNowTripType.flight_HotelTrip(Trip,OriginCityCode,OriginLocation,DestinationCityCode,DestinationLocation,DepartureDate,ReturnDate,Rooms,Adult,Child,
			...
			GoNowResultPage.ResultPageForFlight_Hotel(ApplyFilter,StarRating,PropertyType,RateType,ChooseAnotherFlight,FlightFilter,FareType,AirLine);
		}
		...
		else if (UserType.equalsIgnoreCase("Guest"))
		{
			...
			GoNowCheckoutPage.GuestAdultCheckoutForFlight_Hotel(Adult,Trip,Emailid,AdultTitle,AdultName,Phone,AdultDOBdate,AdultRoomSelection,AdultPassportNumber,
					AdultPassportcountry,AdultExpiryDate,AdultNationality);
			
			GoNowCheckoutPage.GuestChildCheckoutFlight_Hotel(Child,Trip,ChildTitle,ChildName,ChildDOBdate,ChildRoomSelection,ChildPassportNumber,ChildPassportcountry,
					ChildPassportDate,ChildNationality);
			
			GoNowCheckoutPage.GuestInfantCheckoutFlight_Hotel(Infant,Trip,InfantTitle,InfantName,InfantDOBdate,InfantTravellingwith,InfantPassportNumber,InfantPassportcountry,
					InfantPassportDate,InfantNationality);
			
			GoNowPaymentPage.cardPaymentFlight_Hotel(Cardtype,CardNumber,CardName,CardDate,CVV);
			
			GoNowCheckoutPage.GuestBillingDetailsCheckoutFlight_Hotel(BillingTitle,BillingUserName,BillingAddress,BillingCountry,BillingCity);

...
//		test.log(Status.INFO, Text);
	...
	{
		TestBase.wait = new WebDriverWait(QaBrowser.driver,Duration.ofSeconds(time));
	...
	{
		QaBrowser.driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(time));
	}
	
	public static void pageLoad(int time) 
	{
		QaBrowser.driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(time));
	}
	...
	{
		WebDriverWait wait1 = new WebDriverWait(QaBrowser.driver,Duration.ofSeconds(time));
